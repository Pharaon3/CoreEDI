...
import {DailyFrequencyModel, MonthlyFrequencyModel, OneTimeOnlyFrequencyModel, OnEventFrequencyModel, WeeklyFrequencyModel, MinutelyFrequencyModel} from "../FrequencyModel";
...
import FormFrequencyModel from "../FormFrequencyModel.js";
...
Template._frequencyModal.onRendered(function () {
  ...
  templateObject.assignFrequency = function (frequency) {
    if (frequency == "Weekly") {
      ...
      $("#frequencyMinutely").prop("checked", false);
    ...
    if (frequency == "Daily") {
      ...
      document.getElementById("minutelySettings").style.display = "none";
      document.getElementById("oneTimeOnlySettings").style.display = "none";
    }
    ...
    if (frequency == "Minutely") {
      $("#frequencyMinutely").prop("checked", true);
      $("#frequencyDaily").prop("checked", false);
      $("#frequencyMonthly").prop("checked", false);
      $("#frequencyWeekly").prop("checked", false);
      $("#frequencyOnetimeonly").prop("checked", false);
      $("#frequencyOnevent").prop("checked", false);
      document.getElementById("minutelySettings").style.display = "block";
      document.getElementById("weeklySettings").style.display = "none";
      document.getElementById("dailySettings").style.display = "none";
      document.getElementById("monthlySettings").style.display = "none";
      document.getElementById("oneTimeOnlySettings").style.display = "none";
  ...
  templateObject.saveShedule = async e => {
    ...
    if (fxUpdateObject instanceof MonthlyFrequencyModel) {
      ...
      if ($("#dailyWeekdays").prop("checked")) {
        ...
        if(jQuery('.settingsDailySpecType').val() == 2) {
          cronSetting.exceptionType = 1;
          fxUpdateObject.startDate = $("#edtDailyStartDate").val();
          fxUpdateObject.startTime = moment(new Date()).format("HH:mm:ss");

          cronSetting.exceptionDate = fxUpdateObject.getDate();
          console.log(cronSetting.exceptionDate);
        }
    ...
    } else if (fxUpdateObject instanceof MinutelyFrequencyModel) {
      fxUpdateObject.every = parseInt($("#minutelyEveryXMinutes").val());
      fxUpdateObject.startDate = $("#edtMinutelyStartDate").val();
      fxUpdateObject.startTime = $("#edtMinutelyStartTime").val();
    ...
    if (fxUpdateObject.startDate && fxUpdateObject.startTime) {
      if (moment(fxUpdateObject.getDate()).isBefore(new Date()) && !cronSetting.exceptionType) {
        LoadingOverlay.hide();
    ...
    cronSetting.employeeId = FlowRouter.current().queryParams.id;
    cronSetting.selConnectionId = Template.instance().data.selConnectionId;

    cronSetting.buildParsedText();
    try {
      Meteor.call("pullRecordCron", cronSetting);
      LoadingOverlay.hide(0);
      swal({title: "Success", text: "Update was Scheduled Successfully", type: "success", showCancelButton: false, confirmButtonText: "OK"}).then(() => {
...
Template._frequencyModal.events({
  ...
  'click input[name="frequencyRadio"]': event => {
    if (event.target.id == "frequencyMonthly") {
    ...
    } else if (event.target.id == "frequencyMinutely") {
      document.getElementById("minutelySettings").style.display = "block";
      document.getElementById("monthlySettings").style.display = "none";
      document.getElementById("weeklySettings").style.display = "none";
      document.getElementById("dailySettings").style.display = "none";
      ...
      fxUpdateObject = new MinutelyFrequencyModel({
        startDate: currentDate.toISOString().substring(0, 10),
        startTime: "08:00:00"
      });

      $("#edtMinutelyStartTime").val(fxUpdateObject.startTime);
      $("#edtMinutelyStartDate").val(fxUpdateObject.startDate);
  ...
  'click input[name="dailyRadio"]': event => {
    if (event.target.id == "dailyEveryDay") {
      ...
      $(".settingsDailySpecType").attr("disabled", false);
    } else {
      $("#frequencyModal").modal("toggle");
    }
  },
  ...
  'change .settingsDailySpecType': function (event) {
    if(event.target.value == 2) {
      jQuery('#dailyEveryXDays').val(1);
      jQuery('#dailyEveryXDays').attr({"min":1});
      jQuery('#edtDailyStartTime')[0].disabled = true;
    } else {
      jQuery('#dailyEveryXDays').val(1);
      jQuery('#dailyEveryXDays').attr({"min": 1});
      jQuery('#edtDailyStartTime')[0].disabled = false;
    }
  },
  'change #dailyEveryXDays': function (event) {
